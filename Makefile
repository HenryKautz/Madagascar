...
# For OS X clang
GCCOPTIMIZE= $(ARCH) -O3 -fprefetch-loop-arrays -funroll-loops  -ftracer  -momit-leaf-frame-pointer #-falign-functions=64

# Sdded -fcommon to handle multiple definition errors in current gcc and clang
CFLAGS = $(VERSION) $(FEATURES) $(CONFIGURATION) $(INSTRUMENT) $(GCCOPTIMIZE) -fcommon -w #-Wall
...
# OS X does not like -static, fails with error on missing -lcrt0.o
LINKFLAGS = $(ARCH)
...
nplan: date lex.yy.c Makefile $(PARSERC) $(OBJ) $(HDR)
	$(CC) $(CFLAGS) $(LINKFLAGS) $(LINKFEATURES) $(INSTRUMENT) $(OBJ) -lm -o $(EXECUTABLE)
